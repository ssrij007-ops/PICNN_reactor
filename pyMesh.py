...
class hcubeMesh(object):
	...
	#print(">>> Inside hcubeMesh.__init__")

	def __init__(self,inletX,inletY,outletX,outletY,inertWallX,inertWallY,reactingWallX,reactingWallY,
		...
		checkGeo(inletX,inletY,outletX,outletY,inertWallX,inertWallY,reactingWallX,reactingWallY,tolJoint)
		# Extract discretization info
		self.ny=inletX.shape[0]; self.nx=reactingWallX.shape[0]
		# Prellocate the physical domain
		#Inlet->Outlet->inertWall->reactingWall
		self.x=np.zeros([self.ny,self.nx])
		self.y=np.zeros([self.ny,self.nx])
		self.x[:,0]=inletX; self.y[:,0]=inletY
		self.x[:,-1]=outletX; self.y[:,-1]=outletY
		self.x[0,:]=inertWallX; self.y[0,:]=inertWallY
		self.x[-1,:]=reactingWallX; self.y[-1,:]=reactingWallY
		#self.x,self.y=linearMap(self.x,self.y,self.h,self.tolMesh)
		nx = self.x.shape[1]
		ny = self.x.shape[0]
		#self.x, self.y = linearMap(self.x, self.y, nx, ny)
		self.x, self.y = linearMap(self.x, self.y, nx, ny)
		...
		#ax.tick_params(axis='both',which='both',bottom=False,inlet=False,top=False,labelbottom=False,labelinlet=False)
		#ax.axis('off')
		...
		dxdxi_ho_inlet=(-11*self.x[:,0:-3]+18*self.x[:,1:-2]-\
			           9*self.x[:,2:-1]+2*self.x[:,3:])/6/self.h
		dxdxi_ho_outlet=(11*self.x[:,3:]-18*self.x[:,2:-1]+\
			           9*self.x[:,1:-2]-2*self.x[:,0:-3])/6/self.h
		dydxi_ho_inlet=(-11*self.y[:,0:-3]+18*self.y[:,1:-2]-\
			           9*self.y[:,2:-1]+2*self.y[:,3:])/6/self.h
		dydxi_ho_outlet=(11*self.y[:,3:]-18*self.y[:,2:-1]+\
			           9*self.y[:,1:-2]-2*self.y[:,0:-3])/6/self.h
		

		dxdeta_ho_inertWall=(-11*self.x[0:-3,:]+18*self.x[1:-2,:]-\
			           9*self.x[2:-1,:]+2*self.x[3:,:])/6/self.h
		dxdeta_ho_reactingWall=(11*self.x[3:,:]-18*self.x[2:-1,:]+\
			           9*self.x[1:-2,:]-2*self.x[0:-3,:])/6/self.h
		dydeta_ho_inertWall=(-11*self.y[0:-3,:]+18*self.y[1:-2,:]-\
			           9*self.y[2:-1,:]+2*self.y[3:,:])/6/self.h
		dydeta_ho_reactingWall=(11*self.y[3:,:]-18*self.y[2:-1,:]+\
			           9*self.y[1:-2,:]-2*self.y[0:-3,:])/6/self.h

		self.dxdxi_ho=np.zeros(self.x.shape)
		self.dxdxi_ho[:,2:-2]=dxdxi_ho_internal
		self.dxdxi_ho[:,0:2]=dxdxi_ho_inlet[:,0:2]
		self.dxdxi_ho[:,-2:]=dxdxi_ho_outlet[:,-2:]

		self.dydxi_ho=np.zeros(self.y.shape)
		self.dydxi_ho[:,2:-2]=dydxi_ho_internal
		self.dydxi_ho[:,0:2]=dydxi_ho_inlet[:,0:2]
		self.dydxi_ho[:,-2:]=dydxi_ho_outlet[:,-2:]

		self.dxdeta_ho=np.zeros(self.x.shape)
		self.dxdeta_ho[2:-2,:]=dxdeta_ho_internal
		self.dxdeta_ho[0:2,:]=dxdeta_ho_inertWall[0:2,:]
		self.dxdeta_ho[-2:,:]=dxdeta_ho_reactingWall[-2:,:]

		self.dydeta_ho=np.zeros(self.y.shape)
		self.dydeta_ho[2:-2,:]=dydeta_ho_internal
		self.dydeta_ho[0:2,:]=dydeta_ho_inertWall[0:2,:]
		self.dydeta_ho[-2:,:]=dydeta_ho_reactingWall[-2:,:]
...
def checkGeo(inletX,inletY,outletX,outletY,inertWallX,inertWallY,reactingWallX,reactingWallY,tolJoint):
	print(arrow+'Check bc nodes!')
	assert len(inletX.shape)==len(inletY.shape)==len(outletX.shape)==\
	       len(outletY.shape)==len(inertWallX.shape)==len(inertWallY.shape)==\
	       len(reactingWallX.shape)==len(reactingWallY.shape)==1,\
	       'all inlet(outlet)X(Y) must be 1d vector!'
	assert np.abs(inletX[0]-inertWallX[0])<tolJoint,errMessageJoint
	assert np.abs(inletX[-1]-reactingWallX[0])<tolJoint,errMessageJoint
	assert np.abs(outletX[0]-inertWallX[-1])<tolJoint,errMessageJoint
	assert np.abs(outletX[-1]-reactingWallX[-1])<tolJoint,errMessageJoint
	assert np.abs(inletY[0]-inertWallY[0])<tolJoint,errMessageJoint
	assert np.abs(inletY[-1]-reactingWallY[0])<tolJoint,errMessageJoint
	assert np.abs(outletY[0]-inertWallY[-1])<tolJoint,errMessageJoint
	assert np.abs(outletY[-1]-reactingWallY[-1])<tolJoint,errMessageJoint
	assert inletX.shape==inletY.shape==outletX.shape==outletY.shape,\
	       errMessageParallel
	assert reactingWallX.shape==reactingWallY.shape==inertWallX.shape==inertWallY.shape,\
	       errMessageParallel
	print(arrow+'BC nodes pass!') #linearmap(self):
        
def linearMap(x, y, h, tol=None):
	assert x.shape == y.shape, "x and y must be the same shape"
	ny, nx = x.shape
	for i in range(1, nx - 1):
		alpha = i / (nx - 1)
		x[:, i] = (1 - alpha) * x[:, 0] + alpha * x[:, -1]
		y[:, i] = (1 - alpha) * y[:, 0] + alpha * y[:, -1]
	for j in range(1, ny - 1):
		beta = j / (ny - 1)
		x[j, :] = (1 - beta) * x[0, :] + beta * x[-1, :]
		y[j, :] = (1 - beta) * y[0, :] + beta * y[-1, :]
	print("Linear mapping applied to structured mesh.")
	return x, y


def plotBC(ax,x,y):
	ax.plot(x[:,0],y[:,0],'-o',color=cinlet)     # inlet BC
	ax.plot(x[:,-1],y[:,-1],'-o',color=coutlet)  # outlet BC
	ax.plot(x[0,:],y[0,:],'-o',color=cinertWall) # inertWall BC
	ax.plot(x[-1,:],y[-1,:],'-o',color=creactingWall) # reactingWall BC
	return ax
...
'''
def linearMap(bottom, top, nx, ny):
    print(">>> Entered linearMap")
    ...
    X = np.outer(xb, 1 - eta) + np.outer(xt, eta)
    Y = np.outer(yb, 1 - eta) + np.outer(yt, eta)
    def linearMap(bottom, top, nx, ny):
